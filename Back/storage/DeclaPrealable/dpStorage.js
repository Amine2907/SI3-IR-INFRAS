...
const downloadPdf = async (filePath) => {
  try {
    ...
    // Ensure that the file path has only one 'declPreal-pdf/' prefix
    const fixedFilePath = filePath.startsWith('declPreal-pdf/') ? filePath : `declPreal-pdf/${filePath}`;
...
const listFiles = async (declPrealId,Sid) => {
  try {
    // Ensure declPrealId is a string when constructing the path
    const folderPath = `declPreal-pdf/${Sid}/${declPrealId}`;
    ...
    return data.map(file => ({
      name: file.name,
      path: `declPreal-pdf/${Sid}/${declPrealId}/${file.name}`,
    }));
  } catch (error) {
...
export default declPrealStorage;
