...
const downloadPdf = async (filePath) => {
  try {
    ...
    // Ensure that the file path has only one 'demrac-pdf/' prefix
    const fixedFilePath = filePath.startsWith('demrac-pdf/') ? filePath : `demrac-pdf/${filePath}`;
...
const listFiles = async (demracId,Sid) => {
  try {
    // Ensure demracId is a string when constructing the path
    const folderPath = `demrac-pdf/${Sid}/${demracId}`;
    ...
    return data.map(file => ({
      name: file.name,
      path: `demrac-pdf/${Sid}/${demracId}/${file.name}`,
    }));
  } catch (error) {
