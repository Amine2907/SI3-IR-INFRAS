...
const TravStorageModal = ({ travId, fetchFiles, onSave, onClose }) => {
  ...
  useEffect(() => {
    const fetchFilesForTraveaux = async () => {
      if (!travId) {
  ...
  const handleConfirmDelete = async () => {
    if (fileToDelete?.path) {
      ...
      try {
        const result = await travStorageService.deleteTravFile(filePath);
  ...
  const handleDownloadFile = async file => {
    try {
      ...
      const result = await travStorageService.downloadTravFile(filePath);
