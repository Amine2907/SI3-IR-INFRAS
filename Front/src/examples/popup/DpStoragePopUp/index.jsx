...
const DpStorageModal = ({ declprea, onSave, onClose }) => {
  const [formData, setFormData] = useState(declprea || {});
  const [files, setFiles] = useState([]); // List of files
  const [errors, setErrors] = useState({});
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        // const response = await declpreaStorageService.generatedeclpreaSignedUrl();
        if (response.success) {
          setFiles(
          ...
          console.error('Error fetching declprea files:', response.error);
        }
      } catch (error) {
        console.error('Error fetching declprea files:', error);
      }
    };
    fetchFiles();
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    // Call the uploaddeclprea function
    // const result = await declpreaStorageService.uploaddeclprea(file);
    if (result.success) {
