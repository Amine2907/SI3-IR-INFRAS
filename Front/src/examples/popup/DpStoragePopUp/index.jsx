...
const declPrealModal = ({ declPreaId, fetchFiles, onSave, onClose }) => {
  ...
  useEffect(() => {
    const fetchFilesForDeclPrea = async () => {
      if (!declPreaId) {
    ...
    fetchFilesForDeclPrea();
  }, [declPreaId, fetchFiles]);
  ...
  const handleConfirmDelete = async () => {
    if (fileToDelete?.path) {
      ...
      try {
        const result = await DeclPraelStorageService.deleteDpFile(filePath);
  ...
  const handleDownloadFile = async file => {
    try {
      ...
      const result = await DeclPraelStorageService.downlaodDpFile(filePath);
    ...
    <div className={styles.modal}>
      <div className={styles.modalContent}>
        <Typography variant="h6" gutterBottom align="center">
          Fichiers du Declaration Prealable
        </Typography>
        {/* List of files */}
...
declPrealModal.propTypes = {
  declPreaId: PropTypes.number,
  fetchFiles: PropTypes.func.isRequired,
  onSave: PropTypes.func.isRequired,
  onClose: PropTypes.func.isRequired,
};
export default declPrealModal;
