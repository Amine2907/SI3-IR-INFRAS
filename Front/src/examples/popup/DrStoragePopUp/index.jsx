...
const DemracStorageModal = ({ demRacId, fetchFiles, onSave, onClose }) => {
  ...
  const handleConfirmDelete = async () => {
    if (fileToDelete?.path) {
      ...
      try {
        const result = await demracsStorageService.deleteDemracsFile(filePath);
  ...
  const handleDownloadFile = async file => {
    try {
      ...
      const result = await demracsStorageService.downlaodDemracsFile(filePath);
    ...
    <div className={styles.modal}>
      <div className={styles.modalContent}>
        <Typography variant="h6" gutterBottom align="center">
          Fichiers du Demande Raccordements
        </Typography>
        {/* List of files */}
