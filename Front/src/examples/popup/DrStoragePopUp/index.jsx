...
const DemRacStorageModal = ({ demracs, onSave, onClose }) => {
  const [formData, setFormData] = useState(demracs || {});
    ...
    const fetchFiles = async () => {
      try {
        if (response.success) {
          ...
          console.error('Error fetching demracs files:', response.error);
        }
      } catch (error) {
        console.error('Error fetching demracs files:', error);
      }
    };
    fetchFiles();
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    // Call the uploaddemracs function
    // const result = await demracsStorageService.uploaddemracs(file);
    if (result.success) {
