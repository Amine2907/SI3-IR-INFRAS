...
const DevisStorageModal = ({ devis, onSave, onClose }) => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        // const response = await devisStorageService.generatedevisSignedUrl();
        if (response.success) {
          setFiles(
          ...
          console.error('Error fetching devis files:', response.error);
        }
      } catch (error) {
        console.error('Error fetching devis files:', error);
      }
    };
    fetchFiles();
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    // Call the uploaddevis function
    // const result = await devisStorageService.uploaddevis(file);
    if (result.success) {
